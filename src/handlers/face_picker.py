...
from src.database.db import get_random_photos
...
async def face_picker(call: types.CallbackQuery, state: FSMContext, dbpool, bot: Bot):
    ...
    if 'photos' not in data or not data['photos']:
        data['photos'] = PairIterator(await get_random_photos(dbpool))
    try:
        photos = next(data['photos'])
    except StopIteration:
        data['people'] = PairIterator(await get_random_photos(dbpool))
        photos = next(data['photos'])
